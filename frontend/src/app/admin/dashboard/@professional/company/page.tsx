...
export default function Settings() {
  ...
  useEffect(() => {
    if (company) {
      setSelected({ id: company.id, name: company.address });
    }
  }, [company]);
  ...
  const handleCompany = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/professional/company/get`,
        {
          method: 'GET',
          headers: {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/professional/company/edit`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${session?.accessToken}`,
      ...
      <form method="post" onSubmit={handleSubmit} className="mx-auto max-w-4xl">
        ...
        <section className="grid gap-x-8 gap-y-6 sm:grid-cols-2">
          <div className="space-y-1">
            ...
            <Text>
              Cette information sera affiché sur votre profil publique. Maximum
              255 caractères.
            </Text>
          </div>
          <div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-green-800">
                Modification de l&apos;entreprise réalisée avec succès.
              </p>
            </div>
